...
public class Controller {
    ...
    void buildDoubleTriangles(Model.DoubleTriangle modelData) {
        modelData.firstTriangle.builtTriangle = buildTriangle(modelData.firstTriangle.mode,
                modelData.firstTriangle.maxWidth, modelData.firstTriangle.rowFiller);
        modelData.secondTriangle.builtTriangle = buildTriangle(modelData.secondTriangle.mode,
                modelData.secondTriangle.maxWidth, modelData.secondTriangle.rowFiller);

    }

    void buildTriangle(Model.Triangle modelData) {
        modelData.builtTriangle = buildTriangle(modelData.mode, modelData.maxWidth, modelData.rowFiller);
    }
    ...
    String[] buildTriangle(int mode, int maxWidth, String rowFiller) {
        ...
        for (int i = 0; i < maxWidth; i++)
            if (mode == 1)
                resultString[i] = rowFiller.repeat(i + 1);  // left-up
            else if (mode == 2)
